...
import { burgerModalPages } from './burgerModalService.tsx'

export const BurgerModal: FC<IMiniModal> = ({ isActive, setIsActive, position, parentLink }) => {
  return (
    <MiniModal
      isActive={isActive}
    ...
    >
      {burgerModalPages
        ? burgerModalPages.map((item, index) => (
            <NavLink
              key={index}
              className={({ isActive }) =>
                cn(
